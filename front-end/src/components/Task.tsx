...
const Task: React.FC<TaskProps> = ({ task, onToggle, onDelete, onUpdate }) => {
  ...
  return (
    <div 
      ref={taskRef}
      className={`w-full max-w-full overflow-hidden rounded-md border-l-4 ${task.is_completed ? 'bg-gray-200/80' : 'bg-white'} transition-all duration-200`}
      style={{ borderLeftColor: taskColor || 'transparent' }}>
      <div 
